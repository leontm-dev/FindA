...
window.addEventListener("keydown", (ev) => {
    if (ev.key === "Enter") {
        if (document.getElementById("loader").checkVisibility({ "checkVisibilityCSS": true })) {
            if (document.getElementById("loader-password").value != "") {
                fetch(`${url}api/dev/password/${document.getElementById("loader-password").value}`, {
                    method: "GET"
                })
                .then(res => {
                    if (res.status == 200) {
...
document.getElementById("edit-send").addEventListener("click", (ev) => {
    ...
    if (title != "") {
        if (infos != "") {
            if (date == "") {
                date = `${new Date().toLocaleTimeString("de-DE")}, ${new Date().toLocaleDateString("de-DE")}`;
            };
            let news = {
                title: title,
                date: date,
